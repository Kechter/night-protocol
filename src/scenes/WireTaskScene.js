...
export class WireTaskScene extends Phaser.Scene {
  ...
  create() {
    ...
    leftColors.forEach((colorIndex, i) => {
      ...
      const socket = this.add
        .rectangle(-offsetX, y, 40, 40, color)
  ...
  stopDrag() {
    ...
    this.rightSockets.forEach((socket) => {
      ...
      if (
        Math.abs(socket.x - localX) < 30 &&
        Math.abs(socket.y - localY) < 30
      ) {
        foundTarget = socket;
      }
    });
    ...
    if (
      foundTarget &&
      foundTarget.colorIndex === this.activeLine.startSocket.colorIndex &&
      !foundTarget.isConnected
    ) {
      // MATCH!
